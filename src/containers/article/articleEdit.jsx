...
class Edit extends Component {
    ...
    */
	handleClose = (removedData,type) => {
		let data = this.state;
	    data[type].data = this.state[type].data.filter(data => data !== removedData);
	    this.setState(data);
	}
	    ...
	    return (
	    	<div className="clear" style={{ overflow: 'auto' }} >
	    		...
	    		<div className="editContent clear">
		    		<Form className="form" onSubmit={handleSubmit.bind(this,form,this.state)}>
				        <FormItem
				          hasFeedback
				          {...formItemLayout}
				          label="标题"
				        >
				        	{getFieldDecorator('title',{rules: [{ required: true, message: '标题不能为空！' }] })(
				        ...
				        >
			        		<div className="category">
						        {category.data.map((category) => {
						          	const tagElem = (
						            	<Tag key={category} closable={true} afterClose={() => this.handleClose(category,'category')}>{category}</Tag>
						          	);
						          	return tagElem;
						        })}
						        ...
						        {!category.inputVisible && <Button size="small" type="dashed" onClick={() => this.showInput('category')}>+ New Category</Button>}
						    </div>
				        </FormItem>
				        <FormItem
				          {...formItemLayout}
				        ...
				        >
			        		<div className="tags">
						        {tag.data.map((tag) => {
						          	const tagElem = (
						            	<Tag key={tag} closable={true} afterClose={() => this.handleClose(tag,'tag')}>{tag}</Tag>
						          	);
						          	return tagElem;
						        })}
						        {tag.inputVisible && (
						        ...
						        {!tag.inputVisible && <Button size="small" type="dashed" onClick={() => this.showInput('tag')}>+ New Tag</Button>}
						    </div>
				        </FormItem>
				        <FormItem
				          {...formItemLayout}
				        ...
				        <FormItem
				          ...
				          label="内容"
				        >
				        	<TextArea autosize={{ minRows: 15, maxRows: 20 }} value={textAreaValue} onChange={this.onChangeTextAreaValue} />
				        </FormItem>
				        <FormItem style={{'textAlign': 'center'}}>
				          	<Button offset={8} type="primary" htmlType="submit">提交</Button>
				        </FormItem>
				    </Form>
				    <div
				    	id="markDown"
...
const mapDispatchToProps = (dispatch, ownProps) => {
	let actions = bindActionCreators({ operateArticle, getArticleList, getCategoryList, getTagList }, dispatch);
